...
def figure3(x_train, y_train, b, w):
    ...
    ax.plot([x0, x0], [b + w * x0, y0 - 0.03], c="r", linewidth=2, linestyle="--")
    ax.arrow(
        x0,
        y0 - 0.03,
        0,
...
def figure4(x_train, y_train, b, w, bs, ws, all_losses):
    ...
    ax1.text(-0.3, 2.5, all_losses[bidx, widx], "Minimum", zdir=(1, 0, 0))
    # Random start
    bidx, widx, _, _ = find_index(b, w, bs, ws)
    ax1.scatter(b, w, all_losses[bidx, widx], c="k")
    # Annotations
    ax1.text(-0.2, -1.5, all_losses[bidx, widx], "Random\n Start", zdir=(1, 0, 0))
...
def figure7(b, w, bs, ws, all_losses):
    ...
    axs[0].set_ylim([-0.1, 6.1])
    axs[0].set_xlabel("w")
    axs[0].set_ylabel("MSE (loss)")
    axs[0].set_title("Fixed: b = {:.2f}".format(fixedb))
    # Red rectangle
    rect = Rectangle((-0.3, 2.3), 0.5, 0.5)
    pc = PatchCollection([rect], facecolor="r", alpha=0.3, edgecolor="r")
    ...
    # Black rectangle
    rect = Rectangle((0.3, 2.3), 0.5, 0.5)
    pc = PatchCollection([rect], facecolor="k", alpha=0.3, edgecolor="k")
...
def figure8(b, w, bs, ws, all_losses):
    ...
    # AFTER
    b_idx_after, w_idx_after, bs_after, ws_after = find_index(
        b + 0.12, w + 0.12, bs, ws
    )

    loss_fixedb, loss_fixedw, loss_before, loss_after_b, loss_after_w = loss_curves(
        b_idx, w_idx, b_idx_after, w_idx_after, all_losses
    )

    # Computes gradient for b
    manual_grad_b, delta_b, delta_mse_b = calc_gradient(
        bs_before, bs_after, loss_before, loss_after_b
    )
    # Computes gradient for w
    manual_grad_w, delta_w, delta_mse_w = calc_gradient(
        ws_before, ws_after, loss_before, loss_after_w
    )

    fig, axs = plt.subplots(1, 2, figsize=(12, 6))

    axs[0].set_ylim([2.3, 2.8])
    axs[0].set_xlim([-0.3, 0.2])
    axs[0].set_xlabel("w")
    axs[0].set_ylabel("MSE (loss)")
    ...
    axs[0].plot(
        [ws_before, ws_before], [loss_after_w, loss_before], "r-", linewidth=1.5
    )

    # Annotations
    axs[0].annotate(
        r"$\delta w = {:.2f}$".format(delta_w), xy=(0.0, 2.7), c="k", fontsize=15
    )
    axs[0].annotate(
        r"$\delta MSE = {:.2f}$".format(delta_mse_w),
        xy=(-0.23, 2.45),
        c="k",
        fontsize=15,
    )
    axs[0].annotate(
        r"$\frac{\delta MSE}{\delta w} \approx" + "{:.2f}$".format(manual_grad_w),
        xy=(-0.05, 2.6),
        c="k",
        fontsize=17,
    )
    ...
    axs[1].plot(
        [bs_before, bs_before], [loss_after_b, loss_before], "k-", linewidth=1.5
    )

    # Annotations
    axs[1].annotate(
        r"$\delta b = {:.2f}$".format(delta_b), xy=(0.67, 2.7), c="k", fontsize=15
    )
    axs[1].annotate(
        r"$\delta MSE = {:.2f}$".format(delta_mse_b),
        xy=(0.45, 2.32),
        c="k",
        fontsize=15,
    )
    axs[1].annotate(
        r"$\frac{\delta MSE}{\delta b} \approx" + "{:.2f}$".format(manual_grad_b),
        xy=(0.62, 2.6),
        c="k",
        fontsize=17,
    )
...
def figure9(x_train, y_train, b, w):
    ...
    # Annotations
    ax.annotate("b = {:.4f} w = {:.4f}".format(b[0], w[0]), xy=(0.2, 0.95), c="g")
...
def figure10(b, w, bs, ws, all_losses, manual_grad_b, manual_grad_w, lr):
    ...
    new_b_idx, new_w_idx, bs_after, ws_after = find_index(
        bs_before - lr * manual_grad_b, ws_before - lr * manual_grad_w, bs, ws
    )
    # Loss before
    loss_before = all_losses[w_idx, b_idx]
    loss_fixedb = all_losses[:, b_idx]
    ...
    axs[0].annotate(
        r"$-\eta \frac{\delta MSE}{\delta b} \approx"
        + "{:.2f}$".format(-lr * manual_grad_w),
        xy=(1, 2),
        c="k",
        fontsize=17,
    )
    ...
    axs[1].annotate(
        r"$-\eta \frac{\delta MSE}{\delta w} \approx"
        + "{:.2f}$".format(-lr * manual_grad_b),
        xy=(1, 2),
        c="k",
        fontsize=17,
    )
...
def figure14(x_train, y_train, b, w, bad_bs, bad_ws, bad_x_train):
    ...
    all_predictions = np.apply_along_axis(
        func1d=lambda x: bad_bs + bad_ws * x, axis=1, arr=x_train
    )
    ...
    bad_all_predictions = np.apply_along_axis(
        func1d=lambda x: bad_bs + bad_ws * x, axis=1, arr=bad_x_train
    )
    ...
    axs[0].plot(
        [fixedb, fixedb], bad_w_range[[0, -1]], linestyle="--", c="r", linewidth=2
    )
    # Horizontal cross section
    axs[0].plot(
        bad_b_range[[0, -1]], [fixedw, fixedw], linestyle="--", c="k", linewidth=2
    )

    # Annotations
    axs[0].annotate("Minimum", xy=(0.5, 0.35), c="k")
    axs[0].annotate("Random Start", xy=(fixedb - 0.6, fixedw - 0.3), c="k")
    ...
    # Annotations
    axs[1].annotate("Minimum", xy=(0.5, 0.35), c="k")
    axs[1].annotate("Random Start", xy=(fixedb - 0.6, fixedw - 0.3), c="k")
...
def figure17(x_train, y_train, scaled_bs, scaled_ws, bad_x_train, scaled_x_train):
    # So we recompute the surface for X_TRAIN using the new ranges
    all_predictions = np.apply_along_axis(
        func1d=lambda x: scaled_bs + scaled_ws * x, axis=1, arr=x_train
    )
    all_errors = all_predictions - y_train.reshape(-1, 1, 1)
    all_losses = (all_errors**2).mean(axis=0)
    ...
    bad_all_predictions = np.apply_along_axis(
        func1d=lambda x: scaled_bs + scaled_ws * x, axis=1, arr=bad_x_train
    )
    bad_all_errors = bad_all_predictions - y_train.reshape(-1, 1, 1)
    bad_all_losses = (bad_all_errors**2).mean(axis=0)
    ...
    scaled_all_predictions = np.apply_along_axis(
        func1d=lambda x: scaled_bs + scaled_ws * x, axis=1, arr=scaled_x_train
    )
    ...
    # Annotations
    axs[0].annotate("Minimum", xy=(0.3, 1.6), c="k")
    ...
    # Loss Surface - BAD
    CS = axs[1].contour(
        scaled_bs[0, :], scaled_ws[:, 0], bad_all_losses, cmap=plt.cm.jet
    )
    axs[1].clabel(CS, inline=1, fontsize=10)
    # Minimum point - BAD
    axs[1].scatter(bad_b_minimum, bad_w_minimum, c="k")
    ...
    # Annotations
    axs[1].annotate("Minimum", xy=(0.3, -0.25), c="k")
    ...
    # Loss Surface - SCALED
    CS = axs[2].contour(
        scaled_bs[0, :], scaled_ws[:, 0], scaled_all_losses, cmap=plt.cm.jet
    )
...
def figure18(x_train, y_train):
    ...
    # Predictions
    ax.plot(
        x_range, yhat_range, label="Final model's predictions", c="k", linestyle="--"
    )

    # Annotations
    ax.annotate(
        "b = {:.4f} w = {:.4f}".format(b_minimum, w_minimum),
        xy=(0.4, 1.5),
        c="k",
        rotation=34,
    )
